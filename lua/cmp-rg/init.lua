    ...
    if event == "stdout" then
      for _, entry in ipairs(data) do
        if entry ~= "" then
          ...
          if
            not ok or result.type == "end" or
              (vim.tbl_contains({"context", "match"}, result.type) and not result.data.lines.text)
           then
            context = {}
    ...
    vim.schedule_wrap(
      function()
        ...
        self.running_job_id =
          vim.fn.jobstart(
          string.format(
            "rg --heading --json --word-regexp -B %d -A %d --color never %s %s%s%s%s %s",
            context_before,
            context_after,
